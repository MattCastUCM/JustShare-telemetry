...
export default class ComputerBaseScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.config = {
            style: style,
            fontFamilyBlack: 'corpid-black'
        }

        // Fondo escalado en cuanto al canvas
        let bg = this.add.image(this.CANVAS_WIDTH / 2, this.CANVAS_HEIGHT / 2, 'computer');
    ...
    translate(id) {
        return this.i18next.t(id, { ns: this.namespace })
    }
    ...
    createButton(x, y, textId, scale, onClick) {
        ...
        let button = new Button(this, x, y, scale, onClick,
            ...
            translation, 
            { font: this.config.fontFamilyBlack, size: 54, style: this.config.style.fontStyle, color: this.config.style.color }, 
    ...
    makeTextAppear(target, textId, duration, useId = true) {
        let translate = textId;
        if(useId) {
            translate = this.translate(translate);
        }
        
        if(target.alpha <= 0) {
            target.setText(translate);
