...
export default class CreditsScene extends ComputerBaseScene {
    ...
    create(params) {
        ...
        let creator1 = this.add.text(title.x, title.y + title.displayHeight + TITLE_OFFSET_Y, "Matt Castellanos", style)
        creator1.setOrigin(0.5, 0.5)
        
        let creator2 = this.add.text(creator1.x, creator1.y + creator1.displayHeight + CREATORS_OFFSET_Y, "Pedro León", style)
        creator2.setOrigin(0.5, 0.5)

        let creator3 = this.add.text(creator2.x, creator2.y + creator2.displayHeight + CREATORS_OFFSET_Y, "Raúl Pérez", style)
